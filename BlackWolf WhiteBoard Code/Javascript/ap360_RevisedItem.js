...
var selelctedRevisedItemGuid;
function approvedRevisedItems_OnClickApproved(primaryControl, selectedItemsId) {

    debugger;

    var isPurhcaseOrderExists = false;
    selelctedRevisedItemGuid = "";
    var count = 1;
    ...
    for (var i = 0; i < selectedItemsId.length; i++) {
        ...
        selelctedRevisedItemGuid += selectedItemsId[i];
        if (count > 1 || count < selectedItemsId.length)
            selelctedRevisedItemGuid += ",";
        count++;
        // console.log(selectedItemsId[i].Id);
    }

    if (selectedItemsId.length > 1)// if only one then no need to remove last character as in case of One record last character is Guid char  
        selelctedRevisedItemGuid = selelctedRevisedItemGuid.substring(0, selelctedRevisedItemGuid.length - 1);
    CallAction(primaryControl);
    //setTimeout(function () {
    //    primaryControl.getControl("Revised_items").refresh();
    //}, 1000);
...
function CallAction(primaryControl) {
    ...
    try {
        ...
        Process.callAction("ap360_ApproveRevisedItem",
            [
                {
                    key: "revsiedItemId",
                    type: Process.Type.String,
                    value: selelctedRevisedItemGuid
                }],
            function (params) {
                //Xrm.Page.getControl("purchaseorderproduct").refresh();
                alert("Selected Revised Items Approved");
                primaryControl.getControl("Revised_items").refresh();
                // Success
